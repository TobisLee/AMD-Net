...
class AMDNet_SKB(BaseModule):
    ...
    def __init__(self,
                 ...
                 pretrained=None,
                 init_cfg=None):
        super(AMDNet_SKB, self).__init__(init_cfg)
        ...
        self.gap_8 = nn.AvgPool2d(8, 8, ceil_mode=True)
        self.gap_4 = nn.AvgPool2d(4, 4, ceil_mode=True)
        self.gap_2 = nn.AvgPool2d(2, 2, ceil_mode=True)
        ...
        # Skip Connection Block (SKB)
        self.skb = nn.ModuleList()
        for i in range(1, num_stages-1):
            skb_block = []
            skb_block.append(
                ConvModule(
                    in_channels=np.sum(enc_channels[:i+1]),
                    ...
                    norm_cfg=norm_cfg,
                    act_cfg=act_cfg))
            self.skb.append((nn.Sequential(*skb_block)))
    ...
    def train(self, mode=True):
        ...
        freezed."""
        super(AMDNet_SKB, self).train(mode)
