...
class AMDNet_DAB(BaseModule):
    ...
    def __init__(self,
                 ...
                 pretrained=None,
                 init_cfg=None):
        super(AMDNet_DAB, self).__init__(init_cfg)
        ...
        # Decoder Attention Block (DAB): (conv3*3)*2 + conv1*1 + CBAM + conv3*3
        # (conv3*3)*2
        self.dab_c1 = nn.ModuleList()
        ...
        for i in range(num_stages-1):
            ...
            dab_block.append(
                ConvModule(
                    ...
                    norm_cfg=norm_cfg,
                    act_cfg=act_cfg))
            self.dab_fuse.append((nn.Sequential(*dab_block)))

        # CBAM
        self.dab_atten = nn.ModuleList()
        for i in range(num_stages-1):
            self.dab_atten.append(CBAMBlock(enc_channels[i]))

        # conv3*3
        self.dab_c2 = nn.ModuleList()
        for i in range(num_stages-1):
            dab_block = []
            dab_block.append(
                ConvModule(
                    ...
                    norm_cfg=norm_cfg,
                    act_cfg=act_cfg))
            self.dab_c2.append((nn.Sequential(*dab_block)))


    def forward(self, x):
        self._check_input_divisible(x)
        x0 = self.encoder[0](x)
        x1 = self.encoder[1](self.pool_2(x0))
        x2 = self.encoder[2](self.pool_2(x1))
        x3 = self.encoder[3](self.pool_2(x2))
        x4 = self.encoder[4](self.pool_2(x3))
    ...
    def train(self, mode=True):
        ...
        freezed."""
        super(AMDNet_DAB, self).train(mode)
